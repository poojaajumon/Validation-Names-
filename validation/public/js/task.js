frappe.ui.form.on('Task', {
    ...
    subject: function(frm) {
        if (!frm.doc.custom_automate) {
            console.log("Subject trigger activated and custom_automate is disabled");
            check_automation_enabled(frm, function(is_enabled) {
                console.log("Automation check result:", is_enabled);
                if (is_enabled) {
                    const formatted_name = format_name(frm.doc.subject);
                    console.log("Formatted Name:", formatted_name);
                    frm.set_value('subject', formatted_name);
                }
            });
        } else {
            console.log("custom_automate is enabled. Skipping Subject trigger.");
        }
    },
...
function check_automation_enabled(frm, callback) {
    ...
    frappe.call({
        ...
        callback: function(response) {
            ...
            const is_enabled = response.message ? response.message.task : false;
            callback(is_enabled);
        }
    });
}
...
frappe.ui.form.on("Task", {
    ...
    priority: function(frm) {
        ...
        if (!frm.dialog_shown) {
            frappe.confirm(
                'Do you want to create a new dictionary entry for subject?',
                function() {
